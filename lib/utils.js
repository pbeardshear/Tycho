...
util.find = function (arr, field, value, index) {
...
util.apply = function (target, source, force) {
...
// Data structures
// ----------------------------------------------------------------------------
// Consistent interface for multiple internal storage methods
util.container = function (type) {
	// Boolean type flag: 0 -> hash/dictionary, 1 -> array
	this.type = Array.isArray(type);
	this.store = this.type ? [] : {};
};
util.container.prototype = {
	// Add an element to the container
	add: function (value, key) {
		if (this.type) {
			if (key) {
				this.store.splice(key, 0, value);
			}
			else {
