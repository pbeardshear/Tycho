	...
	return {
		...
		start: function () {
			...
			if (self.config.multicore) {
				this.broadcastWorkers('start', {}).spread(onStart);
			}
			else {
				onStart.call(this);
			}
		...
		stop: function () {
			if (this.running || self.servers) {
				if (self.config.cluster) {
					this.broadcastWorkers('stop', {}).spread(onStop);
				}
				else {
					onStop.call(this);
				}
		...
		pause: function () {
			if (this.running) {
				if (self.config.cluster) {
					this.broadcastWorkers('pause', {}).spread(onPause);
				}
				else {
					onPause.call(this);
				}
