...
tycho = (function () {
	...
	return {
		...
		createServer: function (config) {
			...
			if (config.routes.server && config.routes.instance && config.routes.connection) {
		...
		createServer: function (config) {
			if (config) {
				...
				_self.autoAssign = config.autoAssign || true;
			...
			this.server = new Server(this.useNames, _self.autoGenerate, _self.autoAssign);
