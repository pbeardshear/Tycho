...
var tycho = (function () {
	...
	function onStart() {
		...
		// TODO: Pass server arguments in the same order that they were provided in the config
		this.fire.apply(this, ['start'].concat(self.servers));
	}
	...
	return {
		...
		start: function () {
			...
			if (self.config.multicore) {
				this.alertWorkers('start', { }, onStart);
			}
			else {
				onStart.call(this);
			}
