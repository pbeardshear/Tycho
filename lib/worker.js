	...
	if (instance) {
		switch (command) {
			...
			case 'join':
				...
				break;
			case 'leave':
				instance.dropConnection(address.connectionID)
				break;
			default:
				break;
		}
...
worker.handleServerMessage = function (message) {
	...
	this.store.send(eventType, this.master, message.payload);
};
...
worker.handleInstanceMessage = function (message) {
	...
	switch (message.type) {
		case 'close':
			...
			break;
		case 'message':
			this.store.send(message.type, this.master, message.payload.data);
			break;
		default:
			break;
	}
