	...
	if (instance) {
		switch (command) {
			...
			case 'join':
				...
				break;
			case 'leave':
				instance.dropConnection(this.connections[address.connectionID]);
				break;
			default:
				break;
		}
...
worker.handleInstanceMessage = function (message) {
	...
	switch (message.type) {
		...
		case 'dropconnection':
			...
			this.store.hdel(instanceID, connectionID, function (err, res) {
				if (!err) {
					this.store.send(eventType, this.master, payload);		
				}
			});
			break;
		default:
