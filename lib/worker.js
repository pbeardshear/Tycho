...
if (cluster.isWorker) {
	...
	var worker = cluster.worker,
		...
		constants = JSON.parse(process.env.constants),
		servers = initializeServers(constants, config);
 ...
 */
function initializeServers(constants, config) {
	var index = config.serverIndex,
		servers = [];

	if (constants.connectionTypes.TCP in index) {
		servers.push( new TCPServer(index[constants.connectionTypes.TCP]) );
	}
	if (constants.connectionTypes.UDP in index) {
		servers.push( new UDPServer(index[constants.connectionTypes.UDP]) );
	}
	if (constants.connectionTypes.WEBSOCKET in index) {
		servers.push( new WSServer(index[constants.connectionTypes.WEBSOCKET]) );
	}
