 ...
 * to TCPConnections.
 */
UDPServer.init = function (channel, config) {
...
UDPServer.onMessage = function (buffer, rinfo) {
	...
	if (connID in this.connections) {
		...
		this.connections[connID].onMessage(buffer, rinfo);
	}
	else {
		// No connection found, create a new virtual connection
		var connection = new UDPConnection(this.processID, connID, this.connectionChannel);
