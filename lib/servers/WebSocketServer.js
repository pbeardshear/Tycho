...
WSServer.init = function (channel, manager, config) {
	if (config.secure) {
		...
		this.httpServer = https.createServer(options, this.onHTTPRequest.bind(this));
	}
	else {
...
WSServer.onHTTPRequest = function (req, res) {
	if (this.enableHTTP) {
		this.manager.emit('server:message', 'httpRequest', req, res);
	}
	else {
		res.writeHead(404);
		res.end();
