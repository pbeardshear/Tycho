 ...
 * Constructor
 */
WSServer.init = function (channel, config) {
	if (config.secure) {
...
WSServer.start = function () {
	...
	this.webSocketServer.on('request', function (request) {
		// this.fire('request');
		self.channel.emit('server:message', {
			type: 'request',
			payload: {
				origin: request.origin
			}
		});
		if (!self.originAllowed(request.origin)) {
...
WSServer.accept = function (request) {
	...
	var websocket = request.accept(null, request.origin),
		connection = new WSConnection(this, this.processID, websocket, this.connectionChannel);
...
WSServer.originAllowed = function (origin) {
	return origin.match(this.origin);
};
