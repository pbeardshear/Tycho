...
Instance.addConnection = function (connection) {
	// Wipe old listeners
	connection.openInstanceChannel(this.connectionChannel);
 ...
 *
 */
Instance.sendMessage = function (connectionID, message, source) {
...
Instance.handleConnectionMessage = function (message) {
	...
	switch (message.type) {
		case 'broadcast':
			this.broadcast(connection, message.payload.data);
			break;
		case 'send':
			this.sendMessage(message.payload.to, message.payload.data, connection);
			break;
		case 'leave':
