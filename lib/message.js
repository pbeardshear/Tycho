...
(function (exports) {
	...
	self.send = function (name, data, callback, connection) {
	...
	self.emit = function (name, message) {
		...
		if (!message.validate || message.validate.call(bin)) {
			self.send(name, message.serialize.call(bin), (message.callback && message.callback.bind(bin)));
		}
	};
	...
	exports.send = function (name, connection, callback) {
		var data = self.environment === 'client' ? connection : null;
		if (self.messages[name]) {
			self.emit(name, self.messages[name], connection);
		}
		else if (data) {
